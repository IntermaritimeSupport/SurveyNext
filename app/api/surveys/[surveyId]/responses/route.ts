...
export async function POST(request: NextRequest, { params }: { params: { surveyId: string } }) {
  ...
  try {
    ...
    if (!survey) {
      return NextResponse.json({ message: 'Encuesta no encontrada o no válida.' }, { status: 404, headers: withCors(origin) });
    }
    if (survey.status !== SurveyStatus.PUBLISHED) {
      return NextResponse.json({ message: 'Esta encuesta no está activa para recibir respuestas.' }, { status: 403, headers: withCors(origin) });
    }

    if (!survey.allowMultipleResponses && clientUserId && !survey.isAnonymous) {
      const existingResponse = await prisma.surveyResponse.findFirst({ where: { surveyId, userId: clientUserId } });
      if (existingResponse) {
        return NextResponse.json({ message: 'Ya has enviado una respuesta para esta encuesta. No se permiten múltiples respuestas.' }, { status: 409, headers: withCors(origin) });
      }
    }
    ...
    for (const answer of answers) {
      ...
      if (!question) continue; // Ignorar preguntas no válidas
      ...
      if (question.required && isAnswerEmpty) {
        return NextResponse.json({ message: `La pregunta '${question.title}' es requerida y no fue respondida.` }, { status: 400, headers: withCors(origin) });
      }
      ...
      switch (question.type) {
        ...
        case QuestionType.EMAIL:
          if (typeof processedValue !== 'string') {
            return NextResponse.json({ message: `La respuesta para '${question.title}' debe ser texto.` }, { status: 400, headers: withCors(origin) });
          }
          if (question.type === QuestionType.EMAIL) {
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailRegex.test(processedValue)) {
              return NextResponse.json({ message: `El formato de email para '${question.title}' es inválido.` }, { status: 400, headers: withCors(origin) });
            }
          }
          if (questionValidation?.maxLength && processedValue.length > questionValidation.maxLength) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' excede ${questionValidation.maxLength} caracteres.` }, { status: 400, headers: withCors(origin) });
          }
          if (questionValidation?.minLength && processedValue.length < questionValidation.minLength) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' requiere al menos ${questionValidation.minLength} caracteres.` }, { status: 400, headers: withCors(origin) });
          }
          if (questionValidation?.pattern && !new RegExp(questionValidation.pattern).test(processedValue)) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' no coincide con el patrón requerido.` }, { status: 400, headers: withCors(origin) });
          }
          break;
        ...
        case QuestionType.RATING:
          ...
          if (processedValue < minVal || processedValue > maxVal) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' debe estar entre ${minVal} y ${maxVal}.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.DATE:
          if (!/^\d{4}-\d{2}-\d{2}$/.test(processedValue) || isNaN(new Date(processedValue).getTime())) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' debe ser fecha válida YYYY-MM-DD.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.TIME:
          if (!/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(processedValue)) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' debe ser formato HH:mm.` }, { status: 400, headers: withCors(origin) });
          }
          break;
        ...
        case QuestionType.DROPDOWN:
          if (!questionOptions?.some(opt => opt.value === processedValue)) {
            return NextResponse.json({ message: `La opción seleccionada para '${question.title}' es inválida.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.CHECKBOXES:
          if (!Array.isArray(processedValue) || !questionOptions) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' debe ser array de opciones.` }, { status: 400, headers: withCors(origin) });
          }
          const validValues = questionOptions.map(opt => opt.value);
          if (!processedValue.every((v: any) => validValues.includes(v))) {
            return NextResponse.json({ message: `Una o más opciones seleccionadas para '${question.title}' son inválidas.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.FILE_UPLOAD:
          if (!processedValue?.fileName || !processedValue?.fileUrl) {
            return NextResponse.json({ message: `La respuesta de archivo para '${question.title}' es inválida.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.SIGNATURE:
          if (typeof processedValue !== 'string' || processedValue.length < 10) {
            return NextResponse.json({ message: `La respuesta de firma para '${question.title}' es inválida.` }, { status: 400, headers: withCors(origin) });
          }
          break;

        case QuestionType.MATRIX:
          if (typeof processedValue !== 'object' || processedValue === null || Array.isArray(processedValue) || Object.keys(processedValue).length === 0) {
            return NextResponse.json({ message: `La respuesta para '${question.title}' (matriz) es inválida.` }, { status: 400, headers: withCors(origin) });
          }
          break;
    ...
    const newSurveyResponse = await prisma.$transaction(async tx => {
      ...
      await tx.surveyAnalytics.upsert({
        ...
        update: { starts: { increment: incrementStarts }, completions: { increment: incrementCompletions } },
        create: { surveyId, date: today, starts: incrementStarts, completions: incrementCompletions, views: 0 },
      });
    ...
    const status = (error.code === 'P2002') ? 409 : (error.code === 'P2003' ? 400 : 500);
    return NextResponse.json({ message: error.message || 'Error interno del servidor.', error: error.message }, { status, headers: withCors(origin) });
  }
}
