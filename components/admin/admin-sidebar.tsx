"use client"

import { useAuth } from "@/contexts/auth-context"
import { useLanguage } from "@/hooks/use-language"
import { Button } from "@/components/ui/button"
import { LanguageSelector } from "@/components/language-selector"
import { FileText, Users, Settings, LogOut, Home, MessageSquare, TrendingUp, X } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"
import { cn } from "@/lib/utils"
import { Role } from "@prisma/client"
import { type Dispatch, type SetStateAction } from "react"

interface AdminSidebarProps {
  sidebarOpen: boolean
  setSidebarOpen: Dispatch<SetStateAction<boolean>>
}

export function AdminSidebar({ sidebarOpen, setSidebarOpen }: AdminSidebarProps) {
  const { user, logout } = useAuth()
  const { t } = useLanguage()
  const pathname = usePathname()

  const userRole = user?.role || Role.USER

  const navigation = [
    {
      name: t("admin"),
      href: "/admin",
      icon: Home,
      current: pathname === "/admin",
      roles: ["ADMIN", "SUPERADMIN", Role.MODERATOR]
    },
    {
      name: t("surveys"),
      href: "/admin/surveys",
      icon: FileText,
      current: pathname.startsWith("/admin/surveys"),
      roles: ["ADMIN", "SUPERADMIN", Role.MODERATOR]
    },
    {
      name: t("responses"),
      href: "/admin/responses",
      icon: MessageSquare,
      current: pathname.startsWith("/admin/responses"),
      roles: ["ADMIN", "SUPERADMIN", Role.MODERATOR]
    },
    {
      name: t("reports"),
      href: "/admin/reports",
      icon: TrendingUp,
      current: pathname.startsWith("/admin/reports"),
      roles: ["ADMIN", "SUPERADMIN", Role.MODERATOR]
    },
    {
      name: "Usuarios",
      href: "/admin/users",
      icon: Users,
      current: pathname.startsWith("/admin/users"),
      roles: ["ADMIN", "SUPERADMIN"]
    },
    {
      name: t("settings"),
      href: "/admin/settings",
      icon: Settings,
      current: pathname.startsWith("/admin/settings"),
      roles: ["ADMIN", "SUPERADMIN"]
    },
  ]

  const filteredNavigation = navigation.filter(item => item.roles.includes(userRole));

  return (
    <>
      {/* Overlay para móvil/tablet cuando la barra lateral está abierta */}
      {sidebarOpen && (
        <div
          className="fixed inset-0 z-40 bg-black/70 bg-opacity-50 lg:hidden"
          onClick={() => setSidebarOpen(false)}
        ></div>
      )}

      {/* Contenido de la barra lateral */}
      <div
        className={cn(
          "flex h-full w-64 flex-col bg-white shadow-lg border-r border-slate-200",
          "fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0",
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        {/* Botón de cierre para móvil/tablet (dentro de la barra lateral) */}
        <div className="flex justify-end p-4 lg:hidden">
          <Button
            variant="ghost"
            onClick={() => setSidebarOpen(false)}
            className="text-slate-600 hover:text-slate-900"
          >
            <X className="h-6 w-6" />
            <span className="sr-only">Cerrar barra lateral</span>
          </Button>
        </div>

        {/* Información del usuario */}
        <div className="border-b border-slate-200 p-4">
          <div className="flex items-center space-x-3">
            <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
              <span className="text-sm font-medium text-blue-600">{user?.name?.charAt(0).toUpperCase()}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate">{user?.name}</p>
              <p className="text-xs text-slate-500 truncate">{user?.email}</p>
            </div>
          </div>
        </div>

        {/* Navegación */}
        <nav className="flex-1 space-y-1 p-4">
          {filteredNavigation.map((item) => {
            const Icon = item.icon
            return (
              <Link
                key={item.name}
                href={item.href}
                onClick={() => setSidebarOpen(false)} // Cierra la barra lateral al hacer clic en un enlace
              >
                <Button
                  variant={item.current ? "default" : "ghost"}
                  className={cn(
                    "w-full justify-start gap-3 h-10",
                    item.current
                      ? "bg-blue-600 text-white hover:bg-blue-700"
                      : "text-slate-600 hover:text-slate-900 hover:bg-slate-100",
                  )}
                >
                  <Icon className="h-5 w-5" />
                  {item.name}
                </Button>
              </Link>
            )
          })}
        </nav>

        {/* Pie de página con selector de idioma y cerrar sesión */}
        <div className="border-t border-slate-200 p-4 space-y-2">
          <LanguageSelector />
          <Button
            variant="outline"
            onClick={() => {
              logout()
              setSidebarOpen(false)
            }}
            className="w-full justify-start gap-3 text-red-600 hover:text-red-700 bg-transparent"
          >
            <LogOut className="h-5 w-5" />
            Cerrar Sesión
          </Button>
        </div>
      </div>
    </>
  )
}